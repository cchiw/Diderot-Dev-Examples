 ...
 *
 * Command: /Users/chariseechiw/diderot/femprime/bin/diderotc --exec fem.diderot
...
inline float * makeTransformHelp(float** C, float* k, float *modOff){
float  result[2];
result[0] = 0-modOff[0] + (C[0][1])*(1.0*k[0] + 2.77555756156289e-17) + (C[0][0])*(-1.0*k[0] - 1.0*k[1] + 1.0);
result[1] = 0-modOff[1] + (C[1][1])*(1.0*k[0] + 2.77555756156289e-17) + (C[1][0])*(-1.0*k[0] - 1.0*k[1] + 1.0);
return(result);
}
float * makeTransform(float * k, int cell, MappTy nM, FloatMapTy pM ){
float *C[2];
getPoints(cell,nM,pM,C);
float random[2] = { 0.0 };
return(makeTransformHelp(C,k,random));
}
float ** helpMakeJacobian(float ** C, float *k){
 float * J[2];J[0][0] = (C[0][0] * (-0.999999999999999)) + (C[0][1] * (0.999999999999999)) + (C[0][2] * (0)) + 0;
J[0][1] = (C[0][0] * (-1.00000000000000)) + (C[0][1] * (8.88178419700125e-16)) + (C[0][2] * (1.00000000000000)) + 0;
J[1][0] = (C[1][0] * (-0.999999999999999)) + (C[1][1] * (0.999999999999999)) + (C[1][2] * (0)) + 0;
J[1][1] = (C[1][0] * (-1.00000000000000)) + (C[1][1] * (8.88178419700125e-16)) + (C[1][2] * (1.00000000000000)) + 0;
return(J);
}
float ** makeJacobian(int cell, MappTy nM, FloatMapTy pM, float *k){
float *C[2];
getPoints(cell,nM,pM,C);
return(helpMakeJacobian(C,k));
}
float ** helpInverseJacobian(float ** C, float *k){
float *J[2];
float t11 = (C[0][0] * (-0.999999999999999)) + (C[0][1] * (0.999999999999999)) + (C[0][2] * (0)) + 0;
float t12 = (C[0][0] * (-1.00000000000000)) + (C[0][1] * (8.88178419700125e-16)) + (C[0][2] * (1.00000000000000)) + 0;
float t21 = (C[1][0] * (-0.999999999999999)) + (C[1][1] * (0.999999999999999)) + (C[1][2] * (0)) + 0;
float t22 = (C[1][0] * (-1.00000000000000)) + (C[1][1] * (8.88178419700125e-16)) + (C[1][2] * (1.00000000000000)) + 0;
...
J[1][1] = t11*det;
return(J);
}float ** makeJacobianInverse(int cell, MappTy nM, FloatMapTy pM, float *k){
float *C[2];
getPoints(cell,nM,pM,C);
return(helpInverseJacobian(C,k));
}
inline float ** helpJIs(float **C){
float k[2];
k[0] = 0.333333333333;
k[1] = 0.333333333333;
float *J[2];
...
J[1][1] = t11*det;
return(J);
}inline float * helpTranslateCoordinates(float **C, float * x0, int itter){
float newpos[2];
 newpos[0] = 0.333333333333;
 newpos[1] = 0.333333333333;
bool notConverged =true;
int i = 0;float change[2] = {0.0 };
float * temp;
float ** J = helpJIs(C);
while ((i < itter) && notConverged){
temp = makeTransformHelp(C,newpos, x0);
change[0]=(J[0][0] * temp[0]) + (J[0][1] * temp[1]) + 0;
change[1]=(J[1][0] * temp[0]) + (J[1][1] * temp[1]) + 0;
float test = (change[0] * change[0]) + (change[1] * change[1]) + 0;
if(test < 1e-12*1e-12){notConverged=false;}newpos[0] -=change[0];
newpos[1] -=change[1];
itter++;
}
return(newpos);
}
fcTy wrapCell (float* vp,  int32_t vL, MappTy nM, FloatMapTy pM)
	{
	int range ;
	if (true) {range = 1;} else {range = 16;}
	float ** C;

		for (int cell = 0;  cell < vL;  cell++) {
		C = getPoints(cell,nM,pM,C);

		newposTy newpos = helpTranslateCoordinates(C,vp,range);
		bool test = (k[0] + 1.0e-14 >= 0) && (k[1] + 1.0e-14 >= 0) && (k[0] + k[1] - 1.0e-14 <= 1);
...
static bool init_globals (world *wrld)
{
    globals *glob = wrld->_globals;
    glob->gv_outSize = 10;
    glob->gv_data = "/Users/chariseechiw/Desktop/work/field_defs/fem/temp1.json";
    return false;
}
static void sample_init (globals *glob, sample_strand *self, int32_t p_ui_0, int32_t p_vi_1)
{
    ...
    MappTy l_celltonode_6 = CellToNode(glob->gv_g);
    tensor_2 _arg_7;
    vpack2(_arg_7, v_5);
    fcTy l_fc_8 = wrapCell(_arg_7.base(), NumCells(glob->gv_g), l_celltonode_6, NodeToPoint(glob->gv_g));
    arrTy l_node_9 = GetNode(GetCell(l_fc_8), l_celltonode_6, NodeToCoord(glob->gv_g));
    MappTy l_celltonode_10 = CellToNode(glob->gv_data);
    tensor_2 _arg_11 = {0.3e1f,0.4e1f,};
    fcTy l_fc_12 = wrapCell(_arg_11.base(), NumCells(glob->gv_data), l_celltonode_10, NodeToPoint(glob->gv_data));
    arrTy l_node_13 = GetNode(GetCell(l_fc_12), l_celltonode_10, NodeToCoord(glob->gv_data));
    self->sv_uu = l_uu_3;
    self->sv_vv = l_vv_4;
    vpack2(self->sv_pos, v_5);
    self->sv_out = ProbePhi(ProbeNodeB(l_node_9, BasisData(glob->gv_g)),
        ProbeInvF(ProbeNodeC(l_node_9, Coordinates(glob->gv_g)), GetPos(l_fc_8))) + ProbePhi(
        ProbeNodeB(l_node_13, BasisData(glob->gv_data)),
        ProbeInvF(ProbeNodeC(l_node_13, Coordinates(glob->gv_data)), GetPos(l_fc_12)));
}
static diderot::strand_status sample_update (sample_strand *self)
{
...
bool world::create_strands ()
{
    ...
    globals *glob = this->_globals;
    int32_t l__t_15 = glob->gv_outSize - 1;
    int lo_0 = 0;
    int lo_1 = 0;
