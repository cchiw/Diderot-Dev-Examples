...
/*! \file X1.cxx
 *
 * Generated from X1.diderot.
 *
 * Command: /Users/chariseechiw/diderot/Diderot-Dev/bin/diderotc --exec X1.diderot
...
static std::string ProgramName = "X1";
...
static void sample_init (sample_strand *self, int32_t p_R_0)
{
    ...
    self->sv_out = std::min(
        std::min(
            std::min(std::min(0.3e1f * 0.1e1f + 0.1e1f * 0.3e1f - 0.0f, 0.7e1f * 0.1e1f + 0.1e1f * 0.7e1f - 0.0f),
                l__t_1 * 0.1e1f + 0.1e1f * l__t_1 - 0.0f), 0.2e1f * 0.1e1f + 0.1e1f * 0.2e1f - 0.0f),
        l_r_2 + l_r_2 - 0.0f);
}
static diderot::strand_status sample_update (sample_strand *self)
{
...
bool world::create_strands ()
{
    ...
    uint32_t ix = 0;
    for (int i_vi_3 = lo_0; i_vi_3 <= hi_1; i_vi_3++) {
        sample_init(this->_strands.strand(ix), i_vi_3);
        ++ix;
    }
    this->swap_state();
