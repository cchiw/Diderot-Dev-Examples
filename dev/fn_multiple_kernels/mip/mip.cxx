...
inline bool inside3Ds1 (vec3 x0, diderot::image3d< float, TY > img)
{
    return 0 < x0[0] && x0[2] < img.size(2) - 1 && 0 < x0[2] && x0[1] < img.size(1) - 1 && 0 < x0[1] && x0[0] < img.size(
        0) - 1;
}
...
static diderot::strand_status raycast_update (globals *glob, raycast_strand *self)
{
    ...
    vec3 v_46 = v_43;
    vec3 v_47 = v_45;
    if (!inside3Ds1(v_45, glob->gv_vol)) {
        l__t_48 = true;
    }
    else {
        bool l__t_49;
    ...
    float l_vY__55 = v_51[1];
    vec2 v_56 = vcons2(0.1e1f, 0.1e1f);
    vec2 v_57 = vcons2(-0.1e1f, 0.1e1f);
    vec2 v_58 = v_56 + vcons2(l_vY__55, l_vY__55 - 0.1e1f) * v_57;
    float l_vX__59 = v_51[0];
    vec2 v_60 = v_56 + vcons2(l_vX__59, l_vX__59 - 0.1e1f) * v_57;
    int32_t l_nX__61 = l_n_52[0];
    int32_t l_nY__62 = l_n_52[1];
    int32_t l_idx_63 = l_n_52[2] + -1;
    int32_t l_sz_64 = glob->gv_vol.size(0);
    int32_t l_sz_65 = glob->gv_vol.size(1);
    int32_t l_mulRes_66 = l_sz_65 * l_idx_63;
    int32_t l_mulRes_67 = l_sz_64 * (l_nY__62 + l_mulRes_66);
    int32_t l_addRes_68 = l_nX__61 + 1;
    int32_t l_addRes_69 = l_nY__62 + 1;
    int32_t l_mulRes_70 = l_sz_64 * (l_addRes_69 + l_mulRes_66);
    int32_t l_mulRes_71 = l_sz_65 * (l_idx_63 + 1);
    int32_t l_mulRes_72 = l_sz_64 * (l_nY__62 + l_mulRes_71);
    int32_t l_mulRes_73 = l_sz_64 * (l_addRes_69 + l_mulRes_71);
    int32_t l_mulRes_74 = l_sz_65 * (l_idx_63 + 2);
    int32_t l_mulRes_75 = l_sz_64 * (l_nY__62 + l_mulRes_74);
    int32_t l_mulRes_76 = l_sz_64 * (l_addRes_69 + l_mulRes_74);
    int32_t l_mulRes_77 = l_sz_65 * (l_idx_63 + 3);
    int32_t l_mulRes_78 = l_sz_64 * (l_nY__62 + l_mulRes_77);
    int32_t l_mulRes_79 = l_sz_64 * (l_addRes_69 + l_mulRes_77);
    self->sv_rayN = l_rayN_42;
    self->sv_out = std::max(self->sv_out,
        vdot4(
            vcons4(0.2e1f, 0.1e1f, 0.1e1f, 0.2e1f) + v_54 * (vcons4(-0.4e1f, 0.0f, 0.0f, 0.4e1f) + v_54 * (vcons4(
                0.25e1f, -0.25e1f, -0.25e1f, 0.25e1f) + v_54 * vcons4(-0.5e0f, 0.15e1f, -0.15e1f, 0.5e0f))),
            vcons4(
                vdot2(v_58,
                    vcons2(
                        vdot2(v_60,
                            vcons2(glob->gv_vol[l_nX__61 + l_mulRes_67], glob->gv_vol[l_addRes_68 + l_mulRes_67])),
                        vdot2(v_60,
                            vcons2(glob->gv_vol[l_nX__61 + l_mulRes_70], glob->gv_vol[l_addRes_68 + l_mulRes_70])))),
                vdot2(v_58,
                    vcons2(
                        vdot2(v_60,
                            vcons2(glob->gv_vol[l_nX__61 + l_mulRes_72], glob->gv_vol[l_addRes_68 + l_mulRes_72])),
                        vdot2(v_60,
                            vcons2(glob->gv_vol[l_nX__61 + l_mulRes_73], glob->gv_vol[l_addRes_68 + l_mulRes_73])))),
                vdot2(v_58,
                    vcons2(
                        vdot2(v_60,
                            vcons2(glob->gv_vol[l_nX__61 + l_mulRes_75], glob->gv_vol[l_addRes_68 + l_mulRes_75])),
                        vdot2(v_60,
                            vcons2(glob->gv_vol[l_nX__61 + l_mulRes_76], glob->gv_vol[l_addRes_68 + l_mulRes_76])))),
                vdot2(v_58,
                    vcons2(
                        vdot2(v_60,
                            vcons2(glob->gv_vol[l_nX__61 + l_mulRes_78], glob->gv_vol[l_addRes_68 + l_mulRes_78])),
                        vdot2(v_60,
                            vcons2(glob->gv_vol[l_nX__61 + l_mulRes_79], glob->gv_vol[l_addRes_68 + l_mulRes_79])))))));
    return diderot::kActive;
}
...
bool world::create_strands ()
{
    ...
    uint32_t ix = 0;
    for (int i_vi_80 = lo_0; i_vi_80 <= hi_1; i_vi_80++) {
        for (int i_ui_81 = lo_2; i_ui_81 <= hi_3; i_ui_81++) {
            float l_op1_e3_l_9_82 = -glob->gv_camUmax;
            float l_op1_e3_l_9_83 = -glob->gv_camVmax;
            raycast_init(this->_globals, this->_strands.strand(ix),
                l_op1_e3_l_9_82 + (static_cast<float>(i_ui_81) - (-0.5e0f)) / (static_cast<float>(glob->gv_iresU) - 0.5e0f - (-0.5e0f)) * (glob->gv_camUmax - l_op1_e3_l_9_82),
                l_op1_e3_l_9_83 + (static_cast<float>(i_vi_80) - (-0.5e0f)) / (static_cast<float>(glob->gv_iresV) - 0.5e0f - (-0.5e0f)) * (glob->gv_camVmax - l_op1_e3_l_9_83));
            ++ix;
        }
    }
    this->swap_state();
