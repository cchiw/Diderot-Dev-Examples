...
/*! \file clerp3.cxx
 *
 * Generated from clerp3.diderot.
 *
 * Command: /Users/chariseechiw/diderot/vis15/bin/diderotc --exec clerp3.diderot
...
static std::string ProgramName = "clerp3";
...
{
    vec2 v_2 = vcons2(0.1e1f, 0.8e1f);
    vec2 v_3 = vcons2(0.9e1f, 0.7e1f);
    float l__t_4 = v_2[0];
    float l__t_5 = v_3[0];
    vpack2(self->sv_a, v_2);
    vpack2(self->sv_b, v_3);
    self->sv_t = -0.5e0f;
    self->sv_out = clamp(l__t_4, l__t_5, l__t_4 + -0.5e0f * (l__t_5 - l__t_4));
}
static diderot::strand_status sample_update (sample_strand *self)
{
...
bool world::create_strands ()
{
    ...
    globals *glob = this->_globals;
    int32_t l__t_8 = glob->gv_outSize - 1;
    int lo_0 = 0;
    int lo_1 = 0;
    int32_t base[2] = {lo_0,lo_1,};
    uint32_t size[2] = {static_cast<uint32_t>(l__t_8 - lo_0 + 1),static_cast<uint32_t>(l__t_8 - lo_1 + 1),};
    if (this->alloc(base, size)) {
        return true;
    }
    uint32_t ix = 0;
    for (int i_vi_9 = lo_0; i_vi_9 <= l__t_8; i_vi_9++) {
        for (int i_ui_10 = lo_1; i_ui_10 <= l__t_8; i_ui_10++) {
            sample_init(this->_strands.strand(ix), i_ui_10, i_vi_9);
            ++ix;
        }
    }
    this->swap_state();
