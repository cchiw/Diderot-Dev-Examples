/*---------- begin cxx-head.in ----------*/
/*! \file dfn_unary_cfe_X1.cxx
 *
 * Generated from dfn_unary_cfe_X1.diderot.
 *
 * Command: /Users/chariseechiw/diderot/Diderot-Dev/bin/diderotc --exec dfn_unary_cfe_X1.diderot
...
static std::string ProgramName = "dfn_unary_cfe_X1";
...
{
    float l_r_2 = 0.1e1f * 0.1e1f * glob->gv__t;
    float l_r_3 = 0.0f * 0.0f * glob->gv__t;
    float l_r_4 = 0.0f * 0.1e1f * glob->gv__t;
    float l_r_5 = 0.1e1f * 0.0f * glob->gv__t;
    self->sv_pos[0] = static_cast<float>(p_ui_0);
    self->sv_pos[1] = static_cast<float>(p_vi_1);
    self->sv_out[0] = l_r_2 + l_r_3;
    self->sv_out[1] = l_r_4 + l_r_5;
    self->sv_out[2] = l_r_5 + l_r_4;
    self->sv_out[3] = l_r_3 + l_r_2;
}
static diderot::strand_status sample_update (sample_strand *self)
{
...
bool world::create_strands ()
{
    ...
    globals *glob = this->_globals;
    int32_t l__t_7 = glob->gv_outSize - 1;
    int lo_0 = 0;
    int lo_1 = 0;
    int32_t base[2] = {lo_0,lo_1,};
    uint32_t size[2] = {static_cast<uint32_t>(l__t_7 - lo_0 + 1),static_cast<uint32_t>(l__t_7 - lo_1 + 1),};
    if (this->alloc(base, size)) {
        return true;
    }
    uint32_t ix = 0;
    for (int i_vi_8 = lo_0; i_vi_8 <= l__t_7; i_vi_8++) {
        for (int i_ui_9 = lo_1; i_ui_9 <= l__t_7; i_ui_9++) {
            sample_init(this->_globals, this->_strands.strand(ix), i_ui_9, i_vi_8);
            ++ix;
        }
    }
    this->swap_state();
