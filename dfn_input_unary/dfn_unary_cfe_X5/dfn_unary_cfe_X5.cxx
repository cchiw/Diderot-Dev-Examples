/*---------- begin cxx-head.in ----------*/
/*! \file dfn_unary_cfe_X5.cxx
 *
 * Generated from dfn_unary_cfe_X5.diderot.
 *
 * Command: /Users/chariseechiw/diderot/Diderot-Dev/bin/diderotc --exec dfn_unary_cfe_X5.diderot
...
static std::string ProgramName = "dfn_unary_cfe_X5";
...
static bool init_globals (world *wrld)
{
    ...
    vpack3(glob->gv_D, vscale3(0.8e1f, vcons3(0.1e1f, 0.3e1f, 0.4e1f)));
    glob->gv_outSize = 10;
    return false;
}
static void sample_init (globals *glob, sample_strand *self, int32_t p_ui_1)
{
    float l__t_2 = static_cast<float>(p_ui_1);
    float l_r_3 = 0.0f * 0.1e1f;
    float l_r_4 = 0.0f * l__t_2;
    float l_r_5 = 0.0f * 0.0f;
    self->sv_pos[0] = 0.0f;
    self->sv_pos[1] = 0.1e1f;
    self->sv_pos[2] = l__t_2;
    self->sv_out[0] = 0.1e1f * 0.0f + l_r_3 + l_r_4 + tensor_ref_3(glob->gv_D)[0];
    self->sv_out[1] = l_r_5 + 0.1e1f * 0.1e1f + l_r_4 + tensor_ref_3(glob->gv_D)[1];
    self->sv_out[2] = l_r_5 + l_r_3 + 0.1e1f * l__t_2 + tensor_ref_3(glob->gv_D)[2];
}
static diderot::strand_status sample_update (sample_strand *self)
{
...
bool world::create_strands ()
{
    ...
    uint32_t ix = 0;
    for (int i_ui_8 = lo_0; i_ui_8 <= hi_1; i_ui_8++) {
        sample_init(this->_globals, this->_strands.strand(ix), i_ui_8);
        ++ix;
    }
    this->swap_state();
