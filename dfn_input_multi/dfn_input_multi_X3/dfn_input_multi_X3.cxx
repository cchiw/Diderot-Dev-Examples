/*---------- begin cxx-head.in ----------*/
/*! \file dfn_input_multi_X3.cxx
 *
 * Generated from dfn_input_multi_X3.diderot.
 *
 * Command: /Users/chariseechiw/diderot/Diderot-Dev/bin/diderotc --exec dfn_input_multi_X3.diderot
...
static std::string ProgramName = "dfn_input_multi_X3";
...
static diderot::strand_status f_update (f_strand *self);
...
struct strand_array {
  ...
  // invoke strand's update method
    ...
    {
        return f_update(this->strand(ix));
    }
...
static void f_init (f_strand *self, int32_t p_i_0)
{
    self->sv_out = 0.0f;
    self->sv_i = p_i_0;
}
static diderot::strand_status f_update (f_strand *self)
{
    float l_r_1 = 0.3e1f * 0.1e1f;
    self->sv_out = 0.1e1f * static_cast<float>(self->sv_i) + l_r_1 + 0.1e1f * 0.2e1f + l_r_1;
...
bool world::create_strands ()
{
    ...
    uint32_t ix = 0;
    for (int i_i_2 = lo_0; i_i_2 <= hi_1; i_i_2++) {
        f_init(this->_strands.strand(ix), i_i_2);
        ++ix;
    }
    this->swap_state();
